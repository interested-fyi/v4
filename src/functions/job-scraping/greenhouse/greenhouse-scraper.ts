...
export default async function greenhouseScraper(
  url: string,
  company_id?: number
) {
  let browser:
  ...
  try {
    ...
    if (boardUrl) {
      ...
      const baseUrl = boardUrl.includes("job-boards.")
        ? "https://job-boards.greenhouse.io"
        : "https://boards.greenhouse.io";

      const extractJobsRegular = (
      ...
      const extractJobsV2 = () => {
        $("div.job-posts").each((_, jobPostSection) => {
          const department = $(jobPostSection)
            .find("h3.section-header")
            .text()
            .trim();
