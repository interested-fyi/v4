...
export default function EndorseDialog({
    ...
    const [form, setForm] = useState<{ relationship: string | null; endorsement: string | null }>({
        relationship: null,
        endorsement: null,
    });
    ...
    const handleEndorse = async (form: { relationship: string | null; endorsement: string | null }) => {
        ...
        if (client && accessToken && form.relationship && form.endorsement) {
            try {
                const schemaEncoder = new SchemaEncoder("string relationship, string endorsement");
            ...
            <DialogContent className='sm:max-w-[425px] bg-[#e1effe] font-body m-auto py-8'>
                <DialogHeader className='flex flex-col gap-3'>
                    ...
                    <div className='text-gray-700 text-sm font-semibold font-body leading-[21px]'>
                        Please indicate your relationship to {user?.name} and the reason for your endorsement.
                    </div>
                </DialogHeader>
